...
func TestKeyValues_NoExternalLogModificationsAllowed(t *testing.T) {
...
func TestKeyValues_NoExternalValueModificationsAllowed(t *testing.T) {
	...
	value, err := os.Create(filepath.Join(os.TempDir(), testsDirname, "1"+GobExt))
	testo.Error(t, err, false)

	_, err = io.WriteString(value, "")
	testo.Error(t, err, false)
